...
public class Missile : Obstacle
{
    ...
    protected const float k_Speed = 4f;
   // public GameObject player,play;
	public void Awake()
	{ 
	...
	{   
		if (m_OwnSegement.manager.isMoving)
		{
			if (m_IsMoving)
			{
                    ...
                    transform.position += transform.forward * (k_Speed*(m_OwnSegement.manager.m_Speed/5.0f)) * Time.deltaTime;
