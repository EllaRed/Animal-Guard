			...
			{if (ob.type == 2)
                {
                    ...
                    controller.trackManager.currentTheme = 
                  ThemeDatabase.GetThemeData(PlayerData.instance.themes[
                     Random.Range(0, PlayerData.instance.themes.Count)
                      ]);
                    controller.trackManager.Clearsegments();
                    portaleffect.SetActive(true);
                    portaleffect.gameObject.GetComponent<PortalEffect>().anim.Play();
                    portaleffect.gameObject.GetComponent<PortalEffect>().audio.Play();
                    Destroy(c.gameObject);
                    Invoke("Turnoff", 2.6f);
                }
				ob.Impacted();
