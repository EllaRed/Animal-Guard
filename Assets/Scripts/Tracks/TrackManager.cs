...
public class TrackManager : MonoBehaviour
{
	...
	public ThemeData currentTheme { get { return m_CurrentThemeData; } set { m_CurrentThemeData = value; } }
   // public int currentlvl;
    ...
    public int accmulti;// accesory multiplier 
    public int hitcount;// enemies hit count
	...
	IEnumerator WaitToStart()
	{
		...
		StartMove();
        if (PlayerData.instance.tutorialcomplete==0)
            {
	...
	void Update ()
	{
		...
		if (parallaxRoot != null && currentTheme.cloudPrefabs.Length > 0)
		{
			while (parallaxRoot.childCount < currentTheme.cloudNumber)
			{
				...
				GameObject obj = Instantiate(currentTheme.cloudPrefabs[UnityEngine.Random.Range(0, currentTheme.cloudPrefabs.Length)]);
				obj.transform.SetParent(parallaxRoot, false);

				obj.transform.localPosition = 
					Vector3.up * (currentTheme.cloudMinimumDistance.y + (UnityEngine.Random.value - 0.5f) * currentTheme.cloudSpread.y) 
					+ Vector3.forward * (lastZ  + (UnityEngine.Random.value - 0.5f) * currentTheme.cloudSpread.z)
					+ Vector3.right * (currentTheme.cloudMinimumDistance.x + (UnityEngine.Random.value - 0.5f) * currentTheme.cloudSpread.x);
        ...
        { // distance milestones achievements
            if (m_TotalWorldDistance >= 2000.0f)
            {
                PlayGamesPlatform.Instance.ReportProgress(
                        GPGSIds.achievement_taking_a_jog,
                        100.0f, (bool success) =>
                        {
                            Debug.Log(" 2k milsetone: " +
                                  success);
                        });
            }
            if (m_TotalWorldDistance >= 5000.0f)
            {
                PlayGamesPlatform.Instance.ReportProgress(
                        GPGSIds.achievement_steadying_the_pace,
                        100.0f, (bool success) =>
                        {
                            Debug.Log(" 5k milsetone: " +
                                  success);
                        });
            }
            if (m_TotalWorldDistance >= 10000.0f)
            {
                PlayGamesPlatform.Instance.ReportProgress(
                        GPGSIds.achievement_feel_the_burn,
                        100.0f, (bool success) =>
                        {
                            Debug.Log(" 10k milsetone: " +
                                  success);
                        });
            }
        ...
        m_Multiplier =( 1 + Mathf.FloorToInt((m_Speed - minSpeed) / (maxSpeed - minSpeed) * speedStep ))*accmulti * PlayerData.instance.rewardmultiplier;
	...
	public void SpawnNewSegment()
	{
		...
		{
            Debug.Log("made new segment");
			m_CurrentZone += 1;
			if (m_CurrentZone >= m_CurrentThemeData.zones.Length)
				m_CurrentZone = 0;
          //  m_CurrentThemeData = ThemeDatabase.GetThemeData(PlayerData.instance.themes[0]);
            m_CurrentZoneDistance = 0;
		}
	...
	public void SpawnObstacle(TrackSegment segment)
	{
		if (segment.possibleObstacles.Length != 0)
		{
			...
			{
				segment.possibleObstacles[UnityEngine.Random.Range(0, segment.possibleObstacles.Length)].Spawn(segment, segment.obstaclePositions[i]);
			}
		}
    ...
    public void Clearsegments()
    {
        ...
        {
            m_PastSegments[i-1].transform.position -= currentPos;
            m_PastSegments[i-1].Cleanup();
            m_PastSegments.RemoveAt(i-1);
        }
